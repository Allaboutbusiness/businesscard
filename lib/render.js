...
.card-admin{display:flex;gap:8px;padding:0 16px 14px}
...
.admin-bar{display:flex;gap:8px;margin-bottom:14px}
...
function renderPostPage(p) {
  ...
  <div class="post-cta">
    ...
    <a class="post-cta-btn" href="/#apply">3회 무료 상담 신청 →</a>
  ...
  fetch('/api/me?_='+Date.now(),{cache:'no-store'}).then(function(r){return r.json()}).then(function(m){
    ...
    var b=document.getElementById('adminBar'); if(b)b.hidden=false;
...
function renderListPage(category, rows) {
  ...
  (function(){
    ...
    fetch('/api/me?_='+Date.now(),{cache:'no-store'}).then(function(r){return r.json()}).then(function(m){
      if(!m||!m.loggedIn)return;
      document.querySelectorAll('.card-admin').forEach(function(el){el.hidden=false;});
