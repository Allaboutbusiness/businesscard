...
.share-row{display:flex;justify-content:flex-end;margin:-2px 0 6px}
.share-btn{display:inline-flex;align-items:center;gap:7px;font-family:inherit;font-size:13px;font-weight:700;
  padding:8px 15px;border-radius:999px;cursor:pointer;color:#F5F3ED;-webkit-tap-highlight-color:transparent;
  border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.06);transition:background .15s,border-color .15s}
.share-btn:hover{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.32)}
.share-btn:active{transform:translateY(1px)}
.share-btn svg{width:15px;height:15px;flex:none}
.share-toast{position:fixed;left:50%;bottom:32px;transform:translateX(-50%) translateY(14px);
  background:#14b8a6;color:#04211d;font-weight:800;font-size:13.5px;padding:11px 20px;border-radius:999px;
  box-shadow:0 10px 30px rgba(0,0,0,.4);opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;z-index:9999}
...
<script src="/measure.js" defer></script></body></html>`;
}
...
function renderPostPage(p) {
  ...
  <article class="panel">
    ...
    <div class="share-row"><button class="share-btn" type="button" data-url="${esc(canonical)}" data-title="${esc(p.title)}" onclick="ownerShare(this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"></circle><circle cx="6" cy="12" r="3"></circle><circle cx="18" cy="19" r="3"></circle><line x1="8.6" y1="10.5" x2="15.4" y2="6.5"></line><line x1="8.6" y1="13.5" x2="15.4" y2="17.5"></line></svg>공유하기</button></div>
  ...
  <div class="share-toast" id="shareToast">링크가 복사되었습니다</div>
  <script>function ownerShare(el){var url=el.getAttribute('data-url'),title=el.getAttribute('data-title')||document.title;if(navigator.share){navigator.share({title:title,url:url}).catch(function(){});return;}var done=function(){var t=document.getElementById('shareToast');if(!t)return;t.classList.add('show');clearTimeout(window.__stt);window.__stt=setTimeout(function(){t.classList.remove('show')},1800)};if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(url).then(done,function(){window.prompt('링크 복사',url)})}else{window.prompt('링크 복사',url)}}</script>`;
