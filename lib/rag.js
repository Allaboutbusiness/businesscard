...
// gemini-flash-latest는 thinking 모델이라 사고가 출력토큰을 소진해 짧은 답이 잘림 → lite(비-thinking)로.
const GEN_MODEL = process.env.RAG_GEN_MODEL || 'gemini-flash-lite-latest';
...
async function generate(systemText, userText, maxTokens) {
  ...
  const body = {
    ...
    // thinkingConfig(thinkingBudget:0)는 현재 gemini-flash-latest에서 400(INVALID_ARGUMENT) → 제거.
    generationConfig: { temperature: 0.3, maxOutputTokens: maxTokens || 300, topP: 0.9 },
    safetySettings: [
