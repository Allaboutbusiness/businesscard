...
// 정부지원사업 매일 매칭 이메일 구독자 테이블(멱등) — 구독 시 lazy 생성해 별도 init 불필요
async function initSubscribers() {
  ...
  // 기존 테이블에 company 컬럼 없으면 추가(멱등) — 이전 배포에서 만든 subscribers 대비
...
// ── 정부지원사업 매칭 구독자 CRUD ──
async function createSubscriber({ email, token, company, sector, sub, region, needs }) {
  const rows = await sql`INSERT INTO subscribers (email, token, company, sector, sub, region, needs)
    VALUES (${email}, ${token}, ${company || null}, ${sector || null}, ${sub || null}, ${region || null}, ${JSON.stringify(needs || [])}::jsonb)
    RETURNING id`;
...
module.exports = { sql, initSchema, createPost, listPosts, listPostsAdmin, listPostsForGrid, getPost, getPostAny, deletePost, setStatus, updatePost, allPublished, postsForKb,
  ...
  initSubscribers, createSubscriber, getSubscriberByToken, listActiveSubscribers, updateSubscriberSeen, deactivateSubscriber };
